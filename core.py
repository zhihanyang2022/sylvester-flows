...
def transform_and_compute_logabsdet(z, Q, R1, R2, b):
    """
...
"""
batch_transform_and_compute_logabsdet
...
transform_conditional_batch = torch.vmap(
    torch.vmap(transform_single_example, (0, 0, 0, 0, 0), 0),
    (1, None, None, None, None),
    1
)
"""

batch_transform_and_compute_logabsdet = torch.vmap(
    torch.vmap(transform_and_compute_logabsdet, (0, None, None, None, None), 0), (0, 0, 0, 0, 0), 0
)


def batch_sample_from_sylvester_flow(μ, σ, Q, R1, R2, b, num_samples, return_z_from_each_layer=False):
    """
    ...
    for k in range(K):
        ...
        out = batch_transform_and_compute_logabsdet(
            z, Q[:, k, :, :], R1[:, k, :, :], R2[:, k, :, :], b[:, k, :]
        )
