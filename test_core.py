...
class TestCore(unittest.TestCase):
    ...
    def test_transform_and_compute_logabsdet(self):
        """
        test shape
        ...
        def transform(z, Q, R1, R2, b):
            return core.transform_and_compute_logabsdet(z, Q, R1, R2, b)[:-1]
        ...
        for seed in range(100):  # ensuring that the difference is small across many random seeds
            ...
            if seed == 0:  # just need to test shape once
                ...
                out = core.transform_and_compute_logabsdet(z, Q, R1, R2, b)
                self.assertEqual(tuple(out.shape), (D+1,))

            logabsdet_efficient = float(core.transform_and_compute_logabsdet(z, Q, R1, R2, b)[-1])
    ...
    def test_batch_transform_and_compute_logabsdet(self):
        """
        test shape
        """
        ...
        out = core.batch_transform_and_compute_logabsdet(z, Q, R1, R2, b)
        
        self.assertEqual(tuple(out.shape), (bs, num_samples, D+1))
        
        
    def test_batch_sample_from_sylvester_flow(self):
        """
        test shape
        """
        ...
        samples, logps = core.batch_sample_from_sylvester_flow(μ, σ, Q, R1, R2, b, num_samples)
